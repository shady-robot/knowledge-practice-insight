...
/*
 * Implmentation notes: randomInteger
 * ----------------------------------
 *  The code for randomInteger produces the number in four steps:
 *
 *  1. Generate a random real number d in te range [0 .. 1).
 ...
 *      high - low + 1
 *
 *  can overflow the integer rang. These calculations must therefore be 
...
double randomReal(double low, double high) {
    ...
    double d = rand() / (double(RAND_MAX) + 1);
    cout << "random number: " << d << endl;
    double s = d * (high - low);
    return low + s;
}

/*
 * Implemtation notes: randomChance
 * --------------------------------
 *  The code for randomChance calls randomReal(0, 1) and checks 
...
/*
 * Implmentation notes: setRandomSeed
 * ----------------------------------
